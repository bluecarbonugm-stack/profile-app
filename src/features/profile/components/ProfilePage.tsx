    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 py-24 md:py-32">
        ...
        <h1 className="mt-4 text-4xl md:text-6xl font-normal text-white max-w-3xl leading-[1.05]">
          Memetakan ekosistem <em className="text-sand">blue&nbsp;carbon</em> Indonesia dengan
          penginderaan jauh dan machine learning.
        </h1>
        <p className="mt-6 max-w-2xl text-white/80 text-base md:text-lg leading-relaxed">
          Blue Carbon Research Group meneliti habitat perairan laut dangkal — terumbu karang, lamun,
          dan mangrove — menggabungkan citra satelit multispektral, survei lapangan, dan klasifikasi
          Random Forest untuk mendukung <em>ocean accounting</em> nasional.
        </p>
    ...
    <section className="mx-auto max-w-[1400px] px-4 py-20">
      <div className="grid md:grid-cols-[1fr_1.4fr] gap-10">
        ...
        <div className="text-muted-foreground leading-relaxed space-y-4">
          <p>
            Blue Carbon Research Group adalah kelompok riset di{" "}
          ...
          <p>
            Karya kami berpangkal pada kebutuhan data spasial habitat perairan dangkal yang akurat
            untuk <em>ocean account</em> nasional. Kami bekerja bersama mitra industri seperti{" "}
            <strong className="text-foreground">PT Mitra Geotama Indonesia</strong> dan lembaga
            pengelola kawasan untuk menghasilkan panduan teknis yang bisa dipakai praktisi di
            lapangan.
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-20">
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-muted-foreground">
            Struktur riset kami mengikuti alur SOP pemetaan habitat perairan dangkal — dari
            pra-pemrosesan citra hingga analisis multi-temporal.
          </p>
        </div>
        <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {PILLARS.map((p, i) => (
            <Card
              key={p.title}
              className="p-5 gap-3 border-border bg-card hover:border-accent/60 transition-colors"
            >
    ...
    <section className="mx-auto max-w-[1400px] px-4 py-20">
      <div className="flex items-end justify-between gap-6 flex-wrap">
        ...
        <p className="text-sm text-muted-foreground max-w-md">
          ...
          Foto & bio lengkap akan ditambahkan oleh tim. Nama di bawah ini adalah placeholder mengacu
          pada kepenulisan Panduan Teknis 2023.
        </p>
      </div>
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-20">
        ...
        <div className="mt-8 divide-y divide-border border border-border rounded-lg bg-card">
          {PUBLICATIONS.map((pub) => (
            <div
              key={pub.title}
              className="p-5 flex flex-col md:flex-row md:items-center gap-3 md:gap-6 hover:bg-muted/40"
            >
        ...
        <div className="relative rounded-xl border border-border overflow-hidden bg-ocean-gradient min-h-[360px]">
          ...
          <div className="absolute bottom-3 left-3 flex gap-2 flex-wrap">
            {[
              ...
              <div
                key={k.l}
                className="text-[10px] flex items-center gap-1.5 bg-black/40 text-white px-2 py-0.5 rounded"
              >
              ...
              {[
                ...
                <div key={r.n}>
                  <div className="flex justify-between text-xs">
                    <span>{r.n}</span>
                    <span className="font-mono">{r.v} ha</span>
                  </div>
                  <div className="h-1.5 bg-muted rounded-full mt-1 overflow-hidden">
                    <div
                      className="h-full rounded-full"
                      style={{ width: `${(r.v / r.max) * 100}%`, background: r.c }}
                    />
                  </div>
                </div>
              ))}
...
function Gallery() {
  const items = [
    ...
    {
      t: "Photo-transect",
      g: "linear-gradient(135deg, oklch(0.35 0.08 230), oklch(0.55 0.11 200))",
    },
    {
      t: "Substrat lamun",
      g: "linear-gradient(135deg, oklch(0.5 0.13 165), oklch(0.65 0.13 145))",
    },
    { t: "Deployment UAV", g: "linear-gradient(135deg, oklch(0.4 0.09 220), oklch(0.7 0.11 45))" },
    {
      t: "CPCe analisis",
      g: "linear-gradient(135deg, oklch(0.32 0.06 240), oklch(0.55 0.15 305))",
    },
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-20">
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {items.map((it) => (
            <div
              key={it.t}
              className="aspect-[3/4] rounded-lg relative overflow-hidden border border-border"
    ...
    <section className="mx-auto max-w-[1400px] px-4 py-16">
      ...
      <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {PARTNERS.map((p) => (
          <div
            key={p}
            className="border border-border rounded-lg p-4 flex flex-col items-start gap-2 hover:border-accent/60"
          >
  ...
  return (
    <section className="border-t border-border bg-ocean-deep text-white">
      <div className="mx-auto max-w-[1400px] px-4 py-20 grid md:grid-cols-2 gap-10">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-teal font-medium">
            08 · Kontak
          </div>
          ...
          <div className="mt-6 space-y-2 text-sm">
            <div className="flex items-center gap-3">
              <MapPin className="h-4 w-4 text-teal" /> Fakultas Geografi UGM, Sekip Utara, Sleman,
              DIY 55281
            </div>
            <div className="flex items-center gap-3">
              <Mail className="h-4 w-4 text-teal" /> bluecarbon@geo.ugm.ac.id
            </div>
            <div className="flex items-center gap-3">
              <GraduationCap className="h-4 w-4 text-teal" /> Departemen Sains Informasi Geografi
            </div>
          </div>
        </div>
        <form
          className="rounded-xl border border-white/15 bg-white/5 backdrop-blur p-6 space-y-3"
          onSubmit={(e) => e.preventDefault()}
        >
          <div className="grid grid-cols-2 gap-3">
            <input
              className="h-10 rounded-md bg-white/10 border border-white/20 px-3 text-sm placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-teal"
              placeholder="Nama"
            />
            <input
              className="h-10 rounded-md bg-white/10 border border-white/20 px-3 text-sm placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-teal"
          ...
          <input
            className="h-10 w-full rounded-md bg-white/10 border border-white/20 px-3 text-sm placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-teal"
            placeholder="Email"
          />
          <textarea
            rows={4}
            className="w-full rounded-md bg-white/10 border border-white/20 px-3 py-2 text-sm placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-teal"
            placeholder="Pesan / topik kolaborasi"
          />
          <Button type="submit" className="bg-teal text-ocean-deep hover:bg-teal/90 w-full">
            Kirim pesan
          </Button>
        </form>
      </div>
    ...
    <footer className="border-t border-border bg-ocean-deep text-white/60">
      <div className="mx-auto max-w-[1400px] px-4 py-6 flex flex-wrap items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2">
          <Layers className="h-3.5 w-3.5" />© {new Date().getFullYear()} Blue Carbon Research Group
          — Fakultas Geografi UGM
        </div>
        <div>
